<?php
  ...
  //if add/update form is submitted
  if(isset($_POST['add_submit'])){
    $brand = sanitize($_POST['brand']);
    //check if brand textbox is blank
    if($brand == ''){
...
<div class="text-center">
  <form class="form-inline" action="brands.php<?=((isset($_GET['edit']))?'?edit='.$edit_id:'');?>" method="post">
    <div class="form-group">
      ...
      ?>                                           <!-- [? = true] [: = false]   --><!-- so the text stays when submit pressed-->
      <label for="brand"><?=((isset($_GET['edit']))?'Edit':'Add A');?> Brand: </label>                            <!-- if there is a value in textbox, ? $_POST['brand'] : '' -->
